...
local function update_monster_data(enemy,last_update)
    local hp = nil
    local quest_status = singletons.questman:get_field("_QuestStatus")
    local is_combat = nil

    if quest_status == 3 or last_update then
    ...
    if monsters.big[enemy].first_hit_time then
        monsters.big[enemy].current_first_hit_dps = calc_dps(
                                                    monsters.big[enemy].max_hp - hp,
                                                    math.floor(singletons.questman:getQuestElapsedTimeSec() - monsters.big[enemy].first_hit_time)
                                                    )
    end

    if not monsters.big[enemy].combat and is_combat or monsters.big[enemy].combat and is_combat and not monsters.big[enemy].combat_start_hp then
        ...
        if not is_player_in_combat() then
            monsters.combat.start = monsters.big[enemy].combat_start_time
        end

        monsters.big[enemy].combat_start_hp = monsters.big[enemy].last_combat_end_hp
        monsters.big[enemy].combat = is_combat

    elseif monsters.big[enemy].combat and not is_combat or monsters.big[enemy].dead then

        monsters.big[enemy].combat = is_combat
        table.insert(
                monsters.big[enemy].combat_times,
                math.floor( singletons.questman:getQuestElapsedTimeSec() - monsters.big[enemy].combat_start_time )
                )
        if not is_player_in_combat() then
            table.insert(
                    monsters.combat.times,
                    math.floor( singletons.questman:getQuestElapsedTimeSec() - monsters.combat.start )
                    )
        end
    ...
    elseif monsters.big[enemy].combat and monsters.big[enemy].combat_start_hp and is_combat then
    ...
    if monsters.big[enemy].dead or not monsters.big[enemy].on_map or quest_status == 3 then
        monsters.big[enemy].last_update = true
    end
end

local function enemy_update(args)
    if not config.current.show_hp and not config.current.show_dps or singletons.questman:get_field("_QuestStatus") ~= 2 then return end
    ...
    if monsters.big[enemy]
    and (not monsters.big[enemy].dead
...
local function transform_time(time,show_ms)
    ...
    elseif quest_status == 3 then
        ...
        if config.current.dps_relative == 2 then
            ...
            if config.current.dps_relative == 4 then
                time = sum(monsters.combat.times)
            elseif config.current.dps_relative == 3 then
...
local function get_string(test)
    ...
    if test then
        ...
        if config.current.show_timer then
            ...
            table.insert(
                    strings,
                    transform_time(singletons.questman:getQuestElapsedTimeSec(),show_ms)
                    )
        end
        if config.current.show_hp
        and monsters.current_target
        and monsters.big[monsters.current_target].current_hp ~= 0
        and singletons.questman:get_field("_QuestStatus") ~= 3 then
            table.insert(
                    strings,
        ...
        if config.current.show_dps then
            ...
            if active_time then
                table.insert(
                        strings,
                        transform_time(active_time,config.current.show_ms)
                        )
            end
...
local function display()
    if not config.current.show_timer and not config.current.show_hp and not config.current.show_dps and not config.current.test
    or
    singletons.questman:get_field("_QuestUIFlow") == 1
    or
    not singletons.guiman:get_field('<refGuiHud>k__BackingField'):get_field('_IsPartVisible') then
        return false
    end
    ...
    if queststatus == 2 or queststatus == 3 and questendflow < 8 or config.current.test then
        return true
    else
        monsters = {
            big={},
...
function()
    if singletons.questman and display() then
        if singletons.questman:get_field("_QuestStatus") == 3 and not monsters.final_update then last_update() end

        timer.draw.text = get_string(config.current.test)

        timer.draw.bg_w,timer.draw.bg_h = timer.draw.font:measure(timer.draw.text)
        if config.current.bg then d2d.fill_rect(config.current.xpos-8, config.current.ypos+1, timer.draw.bg_w+15, timer.draw.bg_h, config.current.bg_color) end
        if config.current.bg then d2d.outline_rect(config.current.xpos-8, config.current.ypos+1, timer.draw.bg_w+15, timer.draw.bg_h, 3, config.current.border_color) end
        d2d.text(timer.draw.font,timer.draw.text, config.current.xpos, config.current.ypos, config.current.font_color)
    end
end)
